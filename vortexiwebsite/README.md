...
### Optional Services
 - Vortexi Gameserver running on Windows Server ( Needed for rendering and games )
 - Vortexi Discord Bot
...
11. **DISCORD_BOT_AUTHTOKEN** - Authorization Token for Vortexi Discord Bot, use random long string
...
29. **DISCOURSE_SSO_ENABLED** - Allows authentication with Vortexi for [Discourse](https://www.discourse.org/) Forums
...
## KeyPair Generation
The Vortexi Backend requires some keys for it to sign and communicate with gameservers, run the script below to generate those keys.
```
